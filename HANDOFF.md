...
1. **Read. DONE.** `/rules` renders all 246 clauses from the committed seed. `src/lib/league/rulebook.ts` holds the numbering, ref resolution, search, highlight, and `referrersOf` (which phase 2 needs to warn before a delete); `rulebookData.ts` mirrors `scheduleData.ts`. Search covers number, title, body, and table cells, and reads through resolved references, so searching "Target" finds the clause that points at it. Typing a number like `4.3` returns that rule and everything under it. Results show a breadcrumb instead of article headings. Tapping a rule number copies a `/rules#rule-<id>` link. Cold loads land via `scroll-margin-top`; in-app navigation centers the clause and flashes it. 16 tests in `tests/rulebook.test.ts` cover renumbering on insert/delete/reorder, refs following a moved target, missing refs rendering visibly, and assertions that the docx's specific defects stay fixed.
