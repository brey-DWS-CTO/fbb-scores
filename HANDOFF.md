...
| Navigation | `src/components/league/AppNav.tsx` | ≥840px: fixed left sidebar with account controls and direct page links, including separate League HQ and History links. Phones: 4 tabs (Keepers, Draft, Teams, Trades) + a grouped MORE sheet. Keeper headers stick while the roster scrolls on desktop and phones; projection notices scroll normally. History tables fit phones; record details retain provenance. Sticky offsets that depend on the bars live at the END of `index.css` and must stay last. |
