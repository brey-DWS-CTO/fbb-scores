...
- A dev-preview harness that injects `PORT` (the Claude Code browser pane does) makes `npm run dev` bind the Express API to Vite's port, so every `/api` call fails with `ECONNREFUSED` through the proxy and the app looks broken. `server/index.ts` already prefers `API_PORT`, but nothing sets it. Workaround: run `API_PORT=3001 npx tsx server/index.ts` yourself alongside the preview. Worth fixing properly by pinning the port in the `dev` script.
